...
data <- read.csv("C:/Users/oyunm/Downloads/bank+marketing/bank/bank-full.csv", header = TRUE, stringsAsFactors = TRUE)
...
### GLM 
```{r}
candidate_fit <- glm(candidate_model, data = candidate_data, family = binomial)
summary(candidate_fit)
exp(cbind(OR = coef(candidate_fit), confint(candidate_fit)))
```
...
```{r, final-complex-plt-1, out.width="100%"}
bank_loess <- bank
...
```{r, final-comple-feat-selection-lasso}


train_model_num <- train_data
train_model_num$subscribed.num <- ifelse(train_data$subscribed=="yes",1,0)


x = model.matrix(subscribed.num ~ poly(age, 2, raw = TRUE) + poly(balance, 2, raw = TRUE) + 
    ...
    2, raw = TRUE):contact + job:education + housing:loan, train_model_num)[,-1]
y= train_model_num$subscribed.num
